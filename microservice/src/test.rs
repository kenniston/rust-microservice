...
pub fn execute_blocking<F, Fut>(future: F)
where
    F: FnOnce() -> Fut,
    Fut: Future<Output = ()>,
{
...
pub mod containers {
    ...
    /// * `network`: Optional. The network to use for the container. If `None`, the default network is used.
    ///
    /// # Returns
    ///
    /// A `Result` containing the URI of the Keycloak instance if successful, or a `TestError` if an error occurred.
